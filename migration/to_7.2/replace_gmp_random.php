...
while($file = array_shift($files)) {
    ...
    while($token = $PHPParser->_next($tokens)) {
        if($token[0] == "T_STRING" && $token[1] == "gmp_random") {
            $to_change[] = $file.":".$token[2].":".$token[1].":change_to_gmp_random_bits";
        }
    }
}
    ...
    if ($handle) {
        ...
        while (($line = fgets($handle)) !== false) {
            if($file_line == $read_line){
                ...
                switch($action){
                    case "change_to_gmp_random_bits":
                        $line = str_replace("gmp_random", "gmp_random_range", $line);
                        if($tmp_line == $line){
                            print "Not changed.\n";
                        } else {
                            print "Changed\n";
